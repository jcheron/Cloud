...
scan=function(folder){
	...
	$.get('scan/files/'+folder, function(data) {
		var response = [data],
			currentPath = '',
			breadcrumbsUrls = [];
		var folders = [],
		...
		function goto(hash) {
			...
			$("[name=activeFolder]").val(data.root+currentPath);
			$('#btDelete').hide();
			$('#panelConfirmDelete').hide();
		}
		...
		function render(data) {
			...
			if(filemanager.hasClass('searching')){
				...
				breadcrumbsUrls.forEach(function (u, i) {
					...
					var img="";
					if(i==0)
						img='<span class="glyphicon glyphicon-hdd" aria-hidden="true"></span>&nbsp;';
					if (i !== breadcrumbsUrls.length - 1) {
						url += '<a href="'+u+'">'+img+'<span class="folderName">' + name[name.length-1] + '</span></a> <span class="arrow">\\</span> ';
					}
					else {
						url += '<span class="folderName">' + img + name[name.length-1] + '</span>';
					}
			...
			fileList.show({'display':'inline-block'});
